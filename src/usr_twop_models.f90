...
!!          variants) and DC-line / weak-coupling-link (DCL_WCL) models.  Built-in
!!          models are resolved through a select-case block.  The prefix "twop_" is
!!          added automatically when absent.

!> @brief Map a two-port model name to its procedure pointer
!! @details Prepends "twop_" to modelname if not already present, then resolves it
