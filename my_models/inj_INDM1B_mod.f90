module inj_INDM1B_mod
    
    double precision :: y(5)=0.d0
    
end module inj_INDM1B_mod
...
double precision function INI_indmach1(name,sbase,SNOM,RS,Lls,LSR,RR,Llr,A,B,LF,vx,vy,ix,iy,typprm)
  ...
  case ('')                             ! compute initial values of motor variables
     ...
     if (SNOM == 0.d0) then
        if (LF == 0.d0) then
           call write_msg_and_stop('inj_INDM1B_mod', ' Induction machine '//trim(name)//': has both SNOM and LF equal to zero')
           stop
        else
     ...
     if(fmax > tol)then
         call write_msg_and_stop('inj_INDM1B_mod', ' INDMACH1 '//trim(name)//': iterative initialization of states failed.')
     endif
  ...
  case('omega0')
      INI_indmach1=y(5)
  case default
      call write_msg_and_stop('inj_INDM1B_mod', ' INI_INDMACH1 called with wrong value of typprm')
  end select
